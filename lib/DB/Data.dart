...
class Data {
  ...
  Future<void> inti() async {
    ...
    if (FileSystemEntity.typeSync(path) == FileSystemEntityType.notFound) {
      ByteData data = await rootBundle.load(join("assets", "db/pro.db"));
  ...
  Future<List<Map>> getTitles() async {
    return  await db.rawQuery('SELECT * from titles order by id');
  }
  Future<List<Map>> getAllFav() async {
    return  await db.rawQuery('SELECT favorite.id,titles.title from favorite,titles where favorite.id=titles.id');
  }

  Future<void> ins_fav(id) async {
    await db.rawInsert("INSERT INTO favorite('id') VALUES('$id')");
  }

  deleteFav_bab(id) async{
    await db.delete("favorite",where:"id='$id'");
  }
  deleteAllFav() async{
    await db.delete("favorite",where:"id not null");
  }
